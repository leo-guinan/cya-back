...
from prelo.models import (
    InvestmentFirm,
    PitchDeck,
    Company,
    DeckReport,
    ConversationDeckUpload,
    InvestorReport,
    RejectionEmail,
    Investor,
    MeetingEmail,
    RequestInfoEmail,
    InviteCoinvestorEmail,
    SourceDeckUpload,
)
...
from prelo.prompts.prompts import (
...
from prelo.tasks import check_for_decks, create_submind_for_investor
    ...
    if user_id:
        ...
        if not company:
            ...
            company = Company.objects.create(
                user_id=user_id, deck_uuid=uuid_for_document
            )
    if client and firm_id and investor_id and user_id and deck_version:
        object_name = (
            f"pitch_decks/{client}/{firm_id}/{investor_id}/{deck_version}/{filename}"
        )
...
def get_scores(request):
    ...
    previous_deck = PitchDeck.objects.filter(
        user_id=pitch_deck.user_id, version=pitch_deck.version - 1
    ).first()
    try:
        ...
        score_object = {
            "market": {
                ...
                "delta": (
                    scores.market_opportunity - previous_deck.scores.market_opportunity
                    if previous_deck
                    else 0
                ),
            },
            "team": {
                ...
                "delta": (
                    scores.team - previous_deck.scores.team if previous_deck else 0
                ),
            },
            "product": {
                ...
                "delta": (
                    scores.product - previous_deck.scores.product
                    if previous_deck
                    else 0
                ),
            },
            "traction": {
                ...
                "delta": (
                    scores.traction - previous_deck.scores.traction
                    if previous_deck
                    else 0
                ),
            },
            "final": {
                ...
                "delta": (
                    scores.final_score - previous_deck.scores.final_score
                    if previous_deck
                    else 0
                ),
            },
...
def send_founder_chat_message(request):
    ...
    path = (
        choose_path_prompt
        | model.bind(function_call={"name": "choose_path"}, functions=functions)
    ...
    path_response = path.invoke({"message": message, "tools": tools_available})
    ...
    if path_response["use_tool"]:
        if path_response["tool_id"] == "1":
            if credits < 2:
                return Response(
                    {
                        "message": "Not enough credits remaining. Please add credits to continue.",
                        "credits_used": 0,
                    }
                )
            ...
            end_time = time.perf_counter()
            print(
                f"Chat with forwardable email writing took {end_time - start_time} seconds"
            )
            record_smd_event(
                {
                    "event": "chat_message",
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system", CHAT_WITH_DECK_SYSTEM_PROMPT),
    ...
    for competitor in competitors:
        try:
            ...
            competitor_info.append(
                {"name": competitor, "url": competitor, "analysis": analysis}
            )
        except Exception as e:
...
def send_mini_chat_message(request):
    ...
    if message == "email_the_founder":
        if investor_report.investment_potential_score < 75:
            rejection_email = RejectionEmail.objects.filter(
                deck_uuid=deck_uuid, investor=investor
            ).first()
            if not rejection_email:
                rejection_email = write_rejection_email(analysis, investor, submind)
            return Response(
                {
                    "email": rejection_email.email,
                    "content": rejection_email.content,
                    "subject": rejection_email.subject,
                }
            )
        elif investor_report.investment_potential_score < 85:
            request_info_email = RequestInfoEmail.objects.filter(
                deck_uuid=deck_uuid, investor=investor
            ).first()
            ...
            return Response(
                {
                    "email": request_info_email.email,
                    "content": request_info_email.content,
                    "subject": request_info_email.subject,
                }
            )
        else:
            meeting_email = MeetingEmail.objects.filter(
                deck_uuid=deck_uuid, investor=investor
            ).first()
            ...
            return Response(
                {
                    "email": meeting_email.email,
                    "content": meeting_email.content,
                    "subject": meeting_email.subject,
                }
            )
...
def send_interview_chat_message(request):
    try:
        ...
        if optional_file:
            ...
            object_name = (
                f"pitch_decks/{client}/{firm_id}/{investor_id}/{optional_file.name}"
            )
            ...
            pitch_deck = PitchDeck.objects.create(
                s3_path=object_name,
                name=optional_file.name,
                uuid=deck_uuid,
                user_id=investor_id,
            )
            Company.objects.create(user_id=investor_id, deck_uuid=deck_uuid)
            ConversationDeckUpload.objects.create(
                conversation_uuid=conversation_uuid, deck_uuid=deck_uuid
            )
            check_for_decks.delay()
            channel_layer = get_channel_layer()
            async_to_sync(channel_layer.group_send)(
                conversation_uuid, {"type": "deck.received", "deck_uuid": deck_uuid}
            )
            ...
            return Response(
                {
                    "message": "Deck has been uploaded and I'm analyzing it now.",
                    "type": "deck_uploaded",
                }
            )
        # Needs a submind to chat with. How does this look in practice?
        ...
        if check_quick:
            if message_type == "email":
                return Response(
                    {
                        "type": "email",
                        "email": check_quick.email,
                        "content": check_quick.content,
                        "subject": check_quick.subject,
                    }
                )
            else:
                chat_history.add_user_message(message)
        ...
        if path_response["use_tool"]:
            ...
            if path_response["tool_id"] == "1":
                ...
                rejection_email = RejectionEmail.objects.filter(
                    deck_uuid=deck.uuid, investor=investor
                ).first()
                if not rejection_email:
                    rejection_email = write_rejection_email(
                        pitch_deck.analysis, investor, submind
                    )
                ...
                if not meeting_email:
                    meeting_email = write_meeting_email(
                        pitch_deck.analysis, investor, submind
                    )
                # return Response({"email": rejection_email.email, "content": rejection_email.content,
                #                  "subject": rejection_email.subject})
                ...
                if not request_info_email:
                    request_info_email = request_more_info(
                        pitch_deck.analysis, investor, submind
                    )
                ...
                pitch_deck = PitchDeck.objects.get(uuid=deck.uuid)
                invite_coinvestor_email = InviteCoinvestorEmail.objects.filter(
                    deck_uuid=deck.uuid, investor=investor
                ).first()
                if not invite_coinvestor_email:
                    invite_coinvestor_email = write_invite_coinvestor(
                        pitch_deck.analysis, investor, submind
                    )
                chat_history.add_user_message(message)
        ...
        custom_prompt = ChatPromptTemplate.from_messages(
            [
                ("system", INTERVIEW_SYSTEM_PROMPT_WITH_CUSTOMIZATION),
                MessagesPlaceholder(variable_name="history"),
                ("human", "{input}"),
            ]
        )
        ...
        investor_answer = investor_message_history.invoke(
            ...
            config={
                "configurable": {"session_id": f"custom_claude_{conversation_uuid}"}
            },
        )
        ...
        return Response(
            {
                "message": "Sorry, an error occurred. Please try again. If the error persists, please contact support."
            }
        )
...
def invite_coinvestor(request):
    ...
    return Response(
        {
            "email": invite_coinvestor_email.email,
            "content": invite_coinvestor_email.content,
            "subject": invite_coinvestor_email.subject,
        }
    )
...
def upload_deck_from_source(request):
    ...
    Company.objects.create(user_id=investor_id, deck_uuid=deck_uuid)
    pitch_deck = PitchDeck.objects.create(
        s3_path=object_name, name=deck_file.name, uuid=deck_uuid, user_id=investor_id
    )
    Company.objects.create(user_id=investor_id, deck_uuid=deck_uuid)
...
def chat_with_deck_at_source(request):
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system", CHAT_WITH_DECK_SYSTEM_PROMPT_AT_SOURCE),
            MessagesPlaceholder(variable_name="history"),
            ("human", "{input}"),
        ]
    )
