...
def validate_team_questions(mind: str, pitch_deck: str) -> str:
    return """
    Evaluate if the output:
    1. Focuses specifically on questions related to the team
...
@validator(validate_team_questions)
@ell.simple(model='gpt-4o-mini', client=OpenAI(api_key=config('OPENAI_API_KEY')))
def prepare_team_questions(mind: str, pitch_deck: str) -> str:
    """
