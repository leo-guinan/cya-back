...
def validate_moat_questions(mind: str, pitch_deck: str) -> str:
    return """
    Evaluate if the output:
    1. Focuses specifically on questions related to the company's moat
    2. Avoids generic questions
    3. References examples from the pitch deck
    4. Demonstrates understanding of moat assessment for early-stage startups
    5. Aligns with the investor's mindset and values
    """

@validator(validate_moat_questions)
@ell.simple(model='gpt-4o-mini', client=OpenAI(api_key=config('OPENAI_API_KEY')))
def prepare_moat_questions(mind: str, pitch_deck: str) -> str:
    """
