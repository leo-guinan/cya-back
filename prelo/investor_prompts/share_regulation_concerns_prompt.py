...
def validate_regulation_concerns(mind: str, pitch_deck: str) -> str:
    return """
    Evaluate if the output:
    1. Focuses specifically on regulation-related concerns
    2. Avoids generic statements
    3. References examples from the pitch deck
    4. Demonstrates understanding of regulatory challenges for early-stage startups
    5. Aligns with the investor's mindset and values
    """

@validator(validate_regulation_concerns)
@ell.simple(model='gpt-4o-mini', client=OpenAI(api_key=config('OPENAI_API_KEY')))
def share_regulation_concerns_prompt(mind: str, pitch_deck: str) -> str:
    """
    ...
    related to Regulation once you've reviewed a pitch deck. Ban generic concerns and focus specifically on
